...
<h1>Update a Commission</h1>

{{ Form::model($commission, array('route' => array('commissions.update', $commission->id), 'method' => 'PUT')) }}
	<ul class="list-unstyled">
		<li class="form-group">
			...
			{{ FormHelper::getDDBoxByArray(Customer::all(),array('fname','lname'),'customer_id',$commission->customer_id) }}
		</li>
		<li class="form-group">
		...
		<a class="btn btn-small btn-info popup" id="AddButton" target="_blank" href="/shows/create">Add Show</a>
			...
			{{ FormHelper::getDDBoxByArray(Show::all(),array('name','state'),'show_id',$commission->show_id) }}
		</li>

		<li class="form-group">
			{{ Form::submit('Update Commission', array('class' => 'btn btn-primary')) }}
		</li>
	</ul>
{{ Form::close() }}
